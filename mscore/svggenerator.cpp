...
public:
    SvgPaintEnginePrivate()
    {
        ...
        attributes.element_uuid = QLatin1String("none");
// UNUSED
...
public:
    ...
    QString elementUuid() const { return d_func()->attributes.element_uuid; }
    void setElementUuid(const QString &elementUuid) {
        d_func()->attributes.element_uuid = elementUuid;
    }
...
void SvgGenerator::setElementUuid(const QString &elementUuid)
{
    Q_D(SvgGenerator);

    d->engine->setElementUuid(elementUuid);
}
...
void SvgPaintEngine::drawPath(const QPainterPath &p)
{
    ...
    stream() << " uuid=\"" << d_func()->attributes.element_uuid << "\"";
